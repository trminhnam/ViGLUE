...
class DataTrainingArguments:
    ...
    word_seg: bool = field(
        default=False,
        metadata={"help": "Whether to use word segmentation for Vietnamese."},
    )
    segmenter: Optional[str] = field(
        default="/media/ddien/minhnam/vncorenlp",
        metadata={"help": "Path to the word segmenter model."},
    )
...
def main():
    ...
    if data_args.word_seg:
        segmenter = py_vncorenlp.VnCoreNLP(
            annotators=["wseg"], save_dir=data_args.segmenter
        )

    def preprocess_function(examples):
        if data_args.word_seg:
            for i, s in enumerate(examples[sentence1_key]):
                out = segmenter.word_segment(s)
                examples[sentence1_key][i] = " ".join(out)
            if sentence2_key is not None:
                for i, s in enumerate(examples[sentence2_key]):
                    out = segmenter.word_segment(s)
                    examples[sentence2_key][i] = " ".join(out)
    ...
    if data_args.task_name is not None:
        if data_args.task_name in {"vnrte", "vsmec", "vsfc", "vtoc"}:
    ...
    if data_args.task_name is not None:
        ...
        kwargs["dataset"] = f"{data_args.dataset_name}/{data_args.task_name.upper()}"
